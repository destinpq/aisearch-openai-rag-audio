                    ...
                    >
                        <div className="mb-2 sm:mb-4 flex items-center justify-between">
                            <div className="flex-1 pr-2">
                                <h2 className="text-base sm:text-xl font-bold text-foreground truncate">{groundingFile.name}</h2>
                                {groundingFile.location && (
                                    <div className="text-xs text-muted-foreground mt-1">
                                        {groundingFile.location.start_line && groundingFile.location.end_line ? (
                                            <span>Lines {groundingFile.location.start_line}-{groundingFile.location.end_line}</span>
                                        ) : null}
                                        {groundingFile.location.pages && groundingFile.location.pages.length > 0 && (
                                            <span className="ml-2">Page{groundingFile.location.pages.length > 1 ? 's' : ''} {groundingFile.location.pages.join(', ')}</span>
                                        )}
                                    </div>
                                )}
                            </div>
