...
export default function useRealTime({
    ...
    const onMessageReceived = (event: MessageEvent<any>) => {
        ...
        switch ((message as unknown as RealtimeMessage).type) {
            case "response.done":
                onReceivedResponseDone?.(message as unknown as ResponseDone);
                break;
            case "response.audio.delta":
                onReceivedResponseAudioDelta?.(message as unknown as ResponseAudioDelta);
                break;
            case "response.audio_transcript.delta":
                onReceivedResponseAudioTranscriptDelta?.(message as unknown as ResponseAudioTranscriptDelta);
                break;
            case "input_audio_buffer.speech_started":
                onReceivedInputAudioBufferSpeechStarted?.(message);
                break;
            case "conversation.item.input_audio_transcription.completed":
                onReceivedInputAudioTranscriptionCompleted?.(message as unknown as ResponseInputAudioTranscriptionCompleted);
                break;
            case "extension.middle_tier_tool_response":
                onReceivedExtensionMiddleTierToolResponse?.(message as unknown as ExtensionMiddleTierToolResponse);
                break;
            case "error":
